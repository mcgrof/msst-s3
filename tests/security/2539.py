...
def test_2539(s3_client, config):
    """Access control 2539"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2539')
        s3_client.create_bucket(bucket_name)

        # Test granular access control
        key = f'access-control/resource-2539.dat'
        ...
        print(f"Access control test 2539: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2539 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2539: {error_code}")
            raise
