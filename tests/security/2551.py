...
def test_2551(s3_client, config):
    """Data masking 2551"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2551')
        s3_client.create_bucket(bucket_name)

        # Test data masking and tokenization
        key = f'masked-data/record-2551.json'

        # Original sensitive data
        original = {
            'customer_id': f'CUST-00002551',
            'email': f'user2551@example.com',
            'phone': f'+1-555-2551',
            'ssn': f'851-41-3551',
            'dob': f'1951-08-04'
        }
        ...
        print(f"Data masking test 2551: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2551 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2551: {error_code}")
            raise
