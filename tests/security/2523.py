...
def test_2523(s3_client, config):
    """Access control 2523"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2523')
        s3_client.create_bucket(bucket_name)

        # Test granular access control
        key = f'access-control/resource-2523.dat'
        ...
        print(f"Access control test 2523: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2523 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2523: {error_code}")
            raise
