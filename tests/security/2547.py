...
def test_2547(s3_client, config):
    """Data masking 2547"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2547')
        s3_client.create_bucket(bucket_name)

        # Test data masking and tokenization
        key = f'masked-data/record-2547.json'

        # Original sensitive data
        original = {
            'customer_id': f'CUST-00002547',
            'email': f'user2547@example.com',
            'phone': f'+1-555-2547',
            'ssn': f'847-37-3547',
            'dob': f'1997-04-28'
        }
        ...
        print(f"Data masking test 2547: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2547 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2547: {error_code}")
            raise
