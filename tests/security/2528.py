...
def test_2528(s3_client, config):
    """Access control 2528"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2528')
        s3_client.create_bucket(bucket_name)

        # Test granular access control
        key = f'access-control/resource-2528.dat'
        ...
        print(f"Access control test 2528: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2528 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2528: {error_code}")
            raise
