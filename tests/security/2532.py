...
def test_2532(s3_client, config):
    """Access control 2532"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2532')
        s3_client.create_bucket(bucket_name)

        # Test granular access control
        key = f'access-control/resource-2532.dat'
        ...
        print(f"Access control test 2532: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2532 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2532: {error_code}")
            raise
