...
def test_2558(s3_client, config):
    """Data masking 2558"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2558')
        s3_client.create_bucket(bucket_name)

        # Test data masking and tokenization
        key = f'masked-data/record-2558.json'

        # Original sensitive data
        original = {
            'customer_id': f'CUST-00002558',
            'email': f'user2558@example.com',
            'phone': f'+1-555-2558',
            'ssn': f'858-48-3558',
            'dob': f'1958-03-11'
        }
        ...
        print(f"Data masking test 2558: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2558 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2558: {error_code}")
            raise
