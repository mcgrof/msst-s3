...
def test_2541(s3_client, config):
    """Data masking 2541"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2541')
        s3_client.create_bucket(bucket_name)

        # Test data masking and tokenization
        key = f'masked-data/record-2541.json'

        # Original sensitive data
        original = {
            'customer_id': f'CUST-00002541',
            'email': f'user2541@example.com',
            'phone': f'+1-555-2541',
            'ssn': f'841-31-3541',
            'dob': f'1991-10-22'
        }
        ...
        print(f"Data masking test 2541: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2541 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2541: {error_code}")
            raise
