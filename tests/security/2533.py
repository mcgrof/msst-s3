...
def test_2533(s3_client, config):
    """Access control 2533"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2533')
        s3_client.create_bucket(bucket_name)

        # Test granular access control
        key = f'access-control/resource-2533.dat'
        ...
        print(f"Access control test 2533: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2533 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2533: {error_code}")
            raise
