...
def test_2565(s3_client, config):
    """Audit logging 2565"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2565')
        s3_client.create_bucket(bucket_name)

        # Test audit logging
        key = f'audit-logs/log-2565.json'

        # Audit event
        audit_event = {
            'event_id': f'AUDIT-000000002565',
            'timestamp': time.time(),
            'event_type': ['ObjectCreated', 'ObjectRead', 'ObjectDeleted', 'BucketModified'][i % 4],
            'user': {
                'id': f'user-65',
                'ip_address': f'192.168.5.5',
                'user_agent': 'aws-cli/2.0.0'
            },
            'resource': {
                'type': 's3_object',
                'bucket': bucket_name,
                'key': f'resource-2565.dat'
            },
            'action': {
        ...
        print(f"Audit logging test 2565: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2565 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2565: {error_code}")
            raise
