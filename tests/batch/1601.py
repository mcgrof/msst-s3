...
def test_1601(s3_client, config):
    """Batch Copy"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1601')
            ...
            if 'Copy' == 'Copy':
                ...
                for obj in objects[:3]:
                    s3_client.client.put_object_tagging(
                        Bucket=bucket_name,
                        Key=obj['Key'],
                        Tagging={'TagSet': [{'Key': 'BatchOp', 'Value': str(1601)}]}
                    )
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1601 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1601: {error_code}")
            raise
