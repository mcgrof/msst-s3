...
def test_2350(s3_client, config):
    """Feature store 2350"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2350')
        s3_client.create_bucket(bucket_name)

        # Test feature store integration
        feature_group = f'feature-store/feature-group-2350/'

        # Feature definitions
        features = {
            'feature_group_name': f'fg_2350',
            'features': [
        ...
        print(f"Feature store test 2350: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2350 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2350: {error_code}")
            raise
