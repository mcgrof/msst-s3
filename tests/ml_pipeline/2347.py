...
def test_2347(s3_client, config):
    """Feature store 2347"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2347')
        s3_client.create_bucket(bucket_name)

        # Test feature store integration
        feature_group = f'feature-store/feature-group-2347/'

        # Feature definitions
        features = {
            'feature_group_name': f'fg_2347',
            'features': [
        ...
        print(f"Feature store test 2347: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2347 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2347: {error_code}")
            raise
