#!/usr/bin/env python3
"""
Test 2391: Inference pipeline 2391
...
def test_2391(s3_client, config):
    """Inference pipeline 2391"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2391')
        s3_client.create_bucket(bucket_name)

        # Test inference pipeline artifacts
        pipeline_key = f'ml-inference/pipelines/pipeline-2391/'

        # Pipeline configuration
        pipeline_config = {
            'pipeline_id': f'PIPELINE-002391',
            'stages': [
        ...
        print(f"Inference pipeline test 2391: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2391 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2391: {error_code}")
            raise
