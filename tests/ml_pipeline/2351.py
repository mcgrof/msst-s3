...
def test_2351(s3_client, config):
    """Feature store 2351"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2351')
        s3_client.create_bucket(bucket_name)

        # Test feature store integration
        feature_group = f'feature-store/feature-group-2351/'

        # Feature definitions
        features = {
            'feature_group_name': f'fg_2351',
            'features': [
        ...
        print(f"Feature store test 2351: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2351 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2351: {error_code}")
            raise
