#!/usr/bin/env python3
"""
Test 2331: Dataset management 2331

Tests ML training dataset management
"""
...
def test_2331(s3_client, config):
    """Dataset management 2331"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2331')
        s3_client.create_bucket(bucket_name)

        # Test training dataset management
        dataset_key = f'ml-datasets/training/dataset-2331/data.tfrecord'

        # Dataset metadata
        dataset_info = {
            'dataset_id': f'DATASET-00002331',
        ...
        print(f"Training dataset test 2331: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2331 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2331: {error_code}")
            raise
