...
def test_2358(s3_client, config):
    """Feature store 2358"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2358')
        s3_client.create_bucket(bucket_name)

        # Test feature store integration
        feature_group = f'feature-store/feature-group-2358/'

        # Feature definitions
        features = {
            'feature_group_name': f'fg_2358',
            'features': [
        ...
        print(f"Feature store test 2358: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2358 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2358: {error_code}")
            raise
