...
def test_2301(s3_client, config):
    """Model storage 2301"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2301')
        s3_client.create_bucket(bucket_name)

        # Test ML model artifact storage
        model_key = f'ml-models/model-2301/artifacts/model.pkl'

        # Model metadata
        model_metadata = {
            'model_id': f'MODEL-002301',
        ...
        metadata_key = f'ml-models/model-2301/metadata.json'
        ...
        print(f"ML model storage test 2301: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2301 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2301: {error_code}")
            raise
