#!/usr/bin/env python3
"""
Test 2013: GDPR compliance 2013
...
def test_2013(s3_client, config):
    """GDPR compliance 2013"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2013')
        s3_client.create_bucket(bucket_name)

        # Test GDPR compliance features
        key = 'gdpr-data-2013.json'

        # Personal data with GDPR markers
        personal_data = {
            'user_id': '2013',
            'email': f'user2013@example.com',
            'gdpr_consent': True,
        ...
        print(f"GDPR compliance test 2013: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2013 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2013: {error_code}")
            raise
