#!/usr/bin/env python3
"""
Test 2004: GDPR compliance 2004
...
def test_2004(s3_client, config):
    """GDPR compliance 2004"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2004')
        s3_client.create_bucket(bucket_name)

        # Test GDPR compliance features
        key = 'gdpr-data-2004.json'

        # Personal data with GDPR markers
        personal_data = {
            'user_id': '2004',
            'email': f'user2004@example.com',
            'gdpr_consent': True,
        ...
        print(f"GDPR compliance test 2004: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2004 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2004: {error_code}")
            raise
