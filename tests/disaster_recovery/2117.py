...
def test_2117(s3_client, config):
    """PITR test 2117"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2117')
        s3_client.create_bucket(bucket_name)

        # Test point-in-time recovery
        key = 'backup-data-2117.json'
        ...
        for v in range(5):
            data = {
                ...
                'data': f'Backup state {v} for item 2117'
            }
        ...
        print(f"Point-in-time recovery test 2117: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2117 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2117: {error_code}")
            raise
