#!/usr/bin/env python3
"""
Test 2127: Cross-region backup 2127

Tests cross-region backup to ap-southeast-1
"""
...
def test_2127(s3_client, config):
    """Cross-region backup 2127"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2127')
        s3_client.create_bucket(bucket_name)

        # Test cross-region backup simulation
        key = f'cross-region-backup-2127.dat'

        # Original data with region metadata
        data = {
            'source_region': 'us-east-1',
            'target_region': 'ap-southeast-1',
            'backup_id': f'BKP00002127',
        ...
        print(f"Cross-region backup to ap-southeast-1: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2127 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2127: {error_code}")
            raise
