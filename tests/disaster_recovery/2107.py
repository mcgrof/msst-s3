...
def test_2107(s3_client, config):
    """PITR test 2107"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2107')
        s3_client.create_bucket(bucket_name)

        # Test point-in-time recovery
        key = 'backup-data-2107.json'
        ...
        for v in range(5):
            data = {
                ...
                'data': f'Backup state {v} for item 2107'
            }
        ...
        print(f"Point-in-time recovery test 2107: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2107 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2107: {error_code}")
            raise
