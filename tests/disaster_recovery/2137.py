#!/usr/bin/env python3
"""
Test 2137: Cross-region backup 2137
...
def test_2137(s3_client, config):
    """Cross-region backup 2137"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2137')
        s3_client.create_bucket(bucket_name)

        # Test cross-region backup simulation
        key = f'cross-region-backup-2137.dat'

        # Original data with region metadata
        data = {
            'source_region': 'us-east-1',
            'target_region': 'us-east-1',
            'backup_id': f'BKP00002137',
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2137 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2137: {error_code}")
            raise
