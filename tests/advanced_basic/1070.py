#!/usr/bin/env python3
"""
Test 1070: Request payment config 1070
...
def test_1070(s3_client, config):
    """Request payment config 1070"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1070')
        ...
        try:
            ...
            s3_client.client.put_bucket_request_payment(
                ...
                RequestPaymentConfiguration={
                    'Payer': 'Requester' if 1070 % 2 == 0 else 'BucketOwner'
                }
            )
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1070 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1070: {error_code}")
            raise
