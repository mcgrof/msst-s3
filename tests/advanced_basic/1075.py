#!/usr/bin/env python3
"""
Test 1075: Request payment config 1075
...
def test_1075(s3_client, config):
    """Request payment config 1075"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1075')
        ...
        try:
            ...
            s3_client.client.put_bucket_request_payment(
                ...
                RequestPaymentConfiguration={
                    'Payer': 'Requester' if 1075 % 2 == 0 else 'BucketOwner'
                }
            )
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1075 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1075: {error_code}")
            raise
