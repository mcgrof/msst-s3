#!/usr/bin/env python3
"""
Test 1076: Request payment config 1076
...
def test_1076(s3_client, config):
    """Request payment config 1076"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1076')
        ...
        try:
            ...
            s3_client.client.put_bucket_request_payment(
                ...
                RequestPaymentConfiguration={
                    'Payer': 'Requester' if 1076 % 2 == 0 else 'BucketOwner'
                }
            )
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1076 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1076: {error_code}")
            raise
