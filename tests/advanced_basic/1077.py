#!/usr/bin/env python3
"""
Test 1077: Request payment config 1077
...
def test_1077(s3_client, config):
    """Request payment config 1077"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1077')
        ...
        try:
            ...
            s3_client.client.put_bucket_request_payment(
                ...
                RequestPaymentConfiguration={
                    'Payer': 'Requester' if 1077 % 2 == 0 else 'BucketOwner'
                }
            )
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1077 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1077: {error_code}")
            raise
