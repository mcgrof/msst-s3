#!/usr/bin/env python3
"""
Test 1066: Request payment config 1066
...
def test_1066(s3_client, config):
    """Request payment config 1066"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1066')
        ...
        try:
            ...
            s3_client.client.put_bucket_request_payment(
                ...
                RequestPaymentConfiguration={
                    'Payer': 'Requester' if 1066 % 2 == 0 else 'BucketOwner'
                }
            )
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1066 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1066: {error_code}")
            raise
