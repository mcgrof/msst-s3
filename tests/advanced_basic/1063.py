#!/usr/bin/env python3
"""
Test 1063: Request payment config 1063
...
def test_1063(s3_client, config):
    """Request payment config 1063"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1063')
        ...
        try:
            ...
            s3_client.client.put_bucket_request_payment(
                ...
                RequestPaymentConfiguration={
                    'Payer': 'Requester' if 1063 % 2 == 0 else 'BucketOwner'
                }
            )
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1063 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1063: {error_code}")
            raise
