#!/usr/bin/env python3
"""
Test 1025: Storage class INTELLIGENT_TIERING
...
def test_1025(s3_client, config):
    """Storage class INTELLIGENT_TIERING"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1025')
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1025 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1025: {error_code}")
            raise
