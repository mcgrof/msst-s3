#!/usr/bin/env python3
"""
Test 1032: Storage class STANDARD_IA
...
def test_1032(s3_client, config):
    """Storage class STANDARD_IA"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1032')
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1032 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1032: {error_code}")
            raise
