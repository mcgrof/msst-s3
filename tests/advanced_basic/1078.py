#!/usr/bin/env python3
"""
Test 1078: Request payment config 1078
...
def test_1078(s3_client, config):
    """Request payment config 1078"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1078')
        ...
        try:
            ...
            s3_client.client.put_bucket_request_payment(
                ...
                RequestPaymentConfiguration={
                    'Payer': 'Requester' if 1078 % 2 == 0 else 'BucketOwner'
                }
            )
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1078 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1078: {error_code}")
            raise
