#!/usr/bin/env python3
"""
Test 1823: Access Point 1823

Tests S3 Access Point configuration 1823
"""
...
def test_1823(s3_client, config):
    """Access Point 1823"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1823')
        s3_client.create_bucket(bucket_name)

        # Test S3 Access Point 1823
        try:
            # Create access point configuration
            access_point_name = f'ap-test-1823'
            ...
            key = f'access-point/test-1823.txt'
            s3_client.put_object(bucket_name, key, io.BytesIO(b'Access point test'))

            print(f"Access point scenario 1823 tested")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1823 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1823: {error_code}")
            raise
