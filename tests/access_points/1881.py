#!/usr/bin/env python3
"""
Test 1881: Access Point 1881

Tests S3 Access Point configuration 1881
"""
...
def test_1881(s3_client, config):
    """Access Point 1881"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1881')
        s3_client.create_bucket(bucket_name)

        # Test S3 Access Point 1881
        try:
            # Create access point configuration
            access_point_name = f'ap-test-1881'
            ...
            key = f'access-point/test-1881.txt'
            s3_client.put_object(bucket_name, key, io.BytesIO(b'Access point test'))

            print(f"Access point scenario 1881 tested")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1881 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1881: {error_code}")
            raise
