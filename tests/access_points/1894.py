#!/usr/bin/env python3
"""
Test 1894: Access Point 1894

Tests S3 Access Point configuration 1894
"""
...
def test_1894(s3_client, config):
    """Access Point 1894"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1894')
        s3_client.create_bucket(bucket_name)

        # Test S3 Access Point 1894
        try:
            # Create access point configuration
            access_point_name = f'ap-test-1894'
            ...
            key = f'access-point/test-1894.txt'
            s3_client.put_object(bucket_name, key, io.BytesIO(b'Access point test'))

            print(f"Access point scenario 1894 tested")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1894 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1894: {error_code}")
            raise
