#!/usr/bin/env python3
"""
Test 1802: Access Point 1802

Tests S3 Access Point configuration 1802
"""
...
def test_1802(s3_client, config):
    """Access Point 1802"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1802')
        s3_client.create_bucket(bucket_name)

        # Test S3 Access Point 1802
        try:
            # Create access point configuration
            access_point_name = f'ap-test-1802'
            ...
            key = f'access-point/test-1802.txt'
            s3_client.put_object(bucket_name, key, io.BytesIO(b'Access point test'))

            print(f"Access point scenario 1802 tested")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1802 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1802: {error_code}")
            raise
