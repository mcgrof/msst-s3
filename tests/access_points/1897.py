#!/usr/bin/env python3
"""
Test 1897: Access Point 1897

Tests S3 Access Point configuration 1897
"""
...
def test_1897(s3_client, config):
    """Access Point 1897"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1897')
        s3_client.create_bucket(bucket_name)

        # Test S3 Access Point 1897
        try:
            # Create access point configuration
            access_point_name = f'ap-test-1897'
            ...
            key = f'access-point/test-1897.txt'
            s3_client.put_object(bucket_name, key, io.BytesIO(b'Access point test'))

            print(f"Access point scenario 1897 tested")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1897 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1897: {error_code}")
            raise
