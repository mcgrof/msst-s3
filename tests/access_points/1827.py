#!/usr/bin/env python3
"""
Test 1827: Access Point 1827

Tests S3 Access Point configuration 1827
"""
...
def test_1827(s3_client, config):
    """Access Point 1827"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1827')
        s3_client.create_bucket(bucket_name)

        # Test S3 Access Point 1827
        try:
            # Create access point configuration
            access_point_name = f'ap-test-1827'
            ...
            key = f'access-point/test-1827.txt'
            s3_client.put_object(bucket_name, key, io.BytesIO(b'Access point test'))

            print(f"Access point scenario 1827 tested")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1827 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1827: {error_code}")
            raise
