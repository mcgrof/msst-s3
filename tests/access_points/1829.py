#!/usr/bin/env python3
"""
Test 1829: Access Point 1829

Tests S3 Access Point configuration 1829
"""
...
def test_1829(s3_client, config):
    """Access Point 1829"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1829')
        s3_client.create_bucket(bucket_name)

        # Test S3 Access Point 1829
        try:
            # Create access point configuration
            access_point_name = f'ap-test-1829'
            ...
            key = f'access-point/test-1829.txt'
            s3_client.put_object(bucket_name, key, io.BytesIO(b'Access point test'))

            print(f"Access point scenario 1829 tested")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1829 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1829: {error_code}")
            raise
