...
def test_2895(s3_client, config):
    """S3 Select 2895"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2895')
        s3_client.create_bucket(bucket_name)

        # Test S3 Select for cost optimization
        key = f's3-select/large-dataset-2895.csv'
        ...
        query_key = f's3-select/queries/query-2895.json'
        s3_client.client.put_object(
            Bucket=bucket_name,
            Key=query_key,
        ...
        print(f"S3 Select optimization test 2895: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2895 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2895: {error_code}")
            raise
