...
def test_2825(s3_client, config):
    """Compression 2825"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2825')
        s3_client.create_bucket(bucket_name)

        # Test compression strategies
        key = f'compressed/data-2825.gz'

        # Generate compressible data
        original_data = (f'Repeated pattern 2825 ' * 1000).encode()
        ...
        print(f"Compression test 2825: {(1 - compression_ratio) * 100:.1f}% savings")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2825 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2825: {error_code}")
            raise
