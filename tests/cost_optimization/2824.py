...
def test_2824(s3_client, config):
    """Compression 2824"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2824')
        s3_client.create_bucket(bucket_name)

        # Test compression strategies
        key = f'compressed/data-2824.gz'

        # Generate compressible data
        original_data = (f'Repeated pattern 2824 ' * 1000).encode()
        ...
        print(f"Compression test 2824: {(1 - compression_ratio) * 100:.1f}% savings")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2824 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2824: {error_code}")
            raise
