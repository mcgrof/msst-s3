#!/usr/bin/env python3
"""
Test 2814: Storage optimization 2814
...
def test_2814(s3_client, config):
    """Storage optimization 2814"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2814')
        s3_client.create_bucket(bucket_name)

        # Test storage class optimization
        key = f'cost-optimized/data-2814.dat'
        data_age_days = i % 365
        ...
        try:
            ...
            print(f"Storage optimization ({storage_class}) test 2814: ✓")
            ...
            if e.response['Error']['Code'] == 'InvalidStorageClass':
                ...
                print(f"Storage optimization (STANDARD fallback) test 2814: ✓")
            else:
                raise
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2814 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2814: {error_code}")
            raise
