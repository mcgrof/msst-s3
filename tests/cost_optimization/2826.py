...
def test_2826(s3_client, config):
    """Compression 2826"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2826')
        s3_client.create_bucket(bucket_name)

        # Test compression strategies
        key = f'compressed/data-2826.gz'

        # Generate compressible data
        original_data = (f'Repeated pattern 2826 ' * 1000).encode()
        ...
        print(f"Compression test 2826: {(1 - compression_ratio) * 100:.1f}% savings")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2826 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2826: {error_code}")
            raise
