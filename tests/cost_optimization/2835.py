...
def test_2835(s3_client, config):
    """Compression 2835"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2835')
        s3_client.create_bucket(bucket_name)

        # Test compression strategies
        key = f'compressed/data-2835.gz'

        # Generate compressible data
        original_data = (f'Repeated pattern 2835 ' * 1000).encode()
        ...
        print(f"Compression test 2835: {(1 - compression_ratio) * 100:.1f}% savings")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2835 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2835: {error_code}")
            raise
