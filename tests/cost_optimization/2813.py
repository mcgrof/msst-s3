#!/usr/bin/env python3
"""
Test 2813: Storage optimization 2813
...
def test_2813(s3_client, config):
    """Storage optimization 2813"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2813')
        s3_client.create_bucket(bucket_name)

        # Test storage class optimization
        key = f'cost-optimized/data-2813.dat'
        data_age_days = i % 365
        ...
        try:
            ...
            print(f"Storage optimization ({storage_class}) test 2813: ✓")
            ...
            if e.response['Error']['Code'] == 'InvalidStorageClass':
                ...
                print(f"Storage optimization (STANDARD fallback) test 2813: ✓")
            else:
                raise
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2813 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2813: {error_code}")
            raise
