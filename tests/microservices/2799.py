#!/usr/bin/env python3
"""
Test 2799: Distributed tracing 2799
...
def test_2799(s3_client, config):
    """Distributed tracing 2799"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2799')
        s3_client.create_bucket(bucket_name)

        # Test distributed tracing
        trace_id = hashlib.md5(f'trace-2799'.encode()).hexdigest()
        ...
        trace = {
            ...
            'spans': [
                {
                    'span_id': hashlib.md5(f'span-2799-{j}'.encode()).hexdigest()[:16],
                    'parent_span_id': hashlib.md5(f'span-2799-{j-1}'.encode()).hexdigest()[:16] if j > 0 else None,
        ...
        print(f"Distributed tracing test 2799: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2799 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2799: {error_code}")
            raise
