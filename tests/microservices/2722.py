...
def test_2722(s3_client, config):
    """API Gateway 2722"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2722')
        s3_client.create_bucket(bucket_name)

        # Test API Gateway configuration
        api_name = f'api-gateway-2722'
        ...
        openapi_spec = {
            ...
            'servers': [
                {'url': f'https://api-2722.example.com/v1'}
            ],
            'paths': {
                f'/resource-{j}': {
                    'get': {
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2722 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2722: {error_code}")
            raise
