#!/usr/bin/env python3
"""
Test 2793: Distributed tracing 2793
...
def test_2793(s3_client, config):
    """Distributed tracing 2793"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2793')
        s3_client.create_bucket(bucket_name)

        # Test distributed tracing
        trace_id = hashlib.md5(f'trace-2793'.encode()).hexdigest()
        ...
        trace = {
            ...
            'spans': [
                {
                    'span_id': hashlib.md5(f'span-2793-{j}'.encode()).hexdigest()[:16],
                    'parent_span_id': hashlib.md5(f'span-2793-{j-1}'.encode()).hexdigest()[:16] if j > 0 else None,
        ...
        print(f"Distributed tracing test 2793: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2793 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2793: {error_code}")
            raise
