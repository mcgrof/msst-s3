...
def test_2717(s3_client, config):
    """Service mesh 2717"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2717')
        s3_client.create_bucket(bucket_name)

        # Test service mesh configuration
        service_name = f'service-auth-2717'
        ...
        print(f"Service mesh test 2717: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2717 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2717: {error_code}")
            raise
