#!/usr/bin/env python3
"""
Test 2790: Distributed tracing 2790
...
def test_2790(s3_client, config):
    """Distributed tracing 2790"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2790')
        s3_client.create_bucket(bucket_name)

        # Test distributed tracing
        trace_id = hashlib.md5(f'trace-2790'.encode()).hexdigest()
        ...
        trace = {
            ...
            'spans': [
                {
                    'span_id': hashlib.md5(f'span-2790-{j}'.encode()).hexdigest()[:16],
                    'parent_span_id': hashlib.md5(f'span-2790-{j-1}'.encode()).hexdigest()[:16] if j > 0 else None,
        ...
        print(f"Distributed tracing test 2790: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2790 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2790: {error_code}")
            raise
