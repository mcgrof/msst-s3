#!/usr/bin/env python3
"""
Test 2794: Distributed tracing 2794
...
def test_2794(s3_client, config):
    """Distributed tracing 2794"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2794')
        s3_client.create_bucket(bucket_name)

        # Test distributed tracing
        trace_id = hashlib.md5(f'trace-2794'.encode()).hexdigest()
        ...
        trace = {
            ...
            'spans': [
                {
                    'span_id': hashlib.md5(f'span-2794-{j}'.encode()).hexdigest()[:16],
                    'parent_span_id': hashlib.md5(f'span-2794-{j-1}'.encode()).hexdigest()[:16] if j > 0 else None,
        ...
        print(f"Distributed tracing test 2794: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2794 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2794: {error_code}")
            raise
