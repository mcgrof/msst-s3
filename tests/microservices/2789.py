#!/usr/bin/env python3
"""
Test 2789: Distributed tracing 2789
...
def test_2789(s3_client, config):
    """Distributed tracing 2789"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2789')
        s3_client.create_bucket(bucket_name)

        # Test distributed tracing
        trace_id = hashlib.md5(f'trace-2789'.encode()).hexdigest()
        ...
        trace = {
            ...
            'spans': [
                {
                    'span_id': hashlib.md5(f'span-2789-{j}'.encode()).hexdigest()[:16],
                    'parent_span_id': hashlib.md5(f'span-2789-{j-1}'.encode()).hexdigest()[:16] if j > 0 else None,
        ...
        print(f"Distributed tracing test 2789: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2789 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2789: {error_code}")
            raise
