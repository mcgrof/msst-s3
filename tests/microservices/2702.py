...
def test_2702(s3_client, config):
    """Service mesh 2702"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2702')
        s3_client.create_bucket(bucket_name)

        # Test service mesh configuration
        service_name = f'service-data-2702'
        ...
        print(f"Service mesh test 2702: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2702 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2702: {error_code}")
            raise
