#!/usr/bin/env python3
"""
Test 2791: Distributed tracing 2791
...
def test_2791(s3_client, config):
    """Distributed tracing 2791"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2791')
        s3_client.create_bucket(bucket_name)

        # Test distributed tracing
        trace_id = hashlib.md5(f'trace-2791'.encode()).hexdigest()
        ...
        trace = {
            ...
            'spans': [
                {
                    'span_id': hashlib.md5(f'span-2791-{j}'.encode()).hexdigest()[:16],
                    'parent_span_id': hashlib.md5(f'span-2791-{j-1}'.encode()).hexdigest()[:16] if j > 0 else None,
        ...
        print(f"Distributed tracing test 2791: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2791 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2791: {error_code}")
            raise
