...
def test_2714(s3_client, config):
    """Service mesh 2714"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2714')
        s3_client.create_bucket(bucket_name)

        # Test service mesh configuration
        service_name = f'service-data-2714'
        ...
        print(f"Service mesh test 2714: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2714 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2714: {error_code}")
            raise
