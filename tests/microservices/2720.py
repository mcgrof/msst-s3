...
def test_2720(s3_client, config):
    """Service mesh 2720"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2720')
        s3_client.create_bucket(bucket_name)

        # Test service mesh configuration
        service_name = f'service-api-2720'
        ...
        print(f"Service mesh test 2720: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2720 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2720: {error_code}")
            raise
