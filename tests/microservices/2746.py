...
def test_2746(s3_client, config):
    """Event-driven 2746"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2746')
        s3_client.create_bucket(bucket_name)

        # Test event-driven patterns
        event_type = ['order.created', 'user.registered', 'payment.processed', 'item.shipped'][i % 4]
        key = f'events/{event_type.replace(".", "/")}/event-2746.json'

        # Event message
        event = {
            'event_id': f'EVT-000000002746',
            'event_type': event_type,
            'timestamp': time.time(),
            'version': '2.0',
            'source': f'service-6',
            'correlation_id': f'CORR-00002746',
            'data': {
                'entity_id': f'ENT-002746',
            ...
            'routing_key': f'{event_type}.medium'
        }
        ...
        print(f"Event-driven test 2746: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2746 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2746: {error_code}")
            raise
