...
def test_1117(s3_client, config):
    """Variable parts 1117"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1117')
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1117 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1117: {error_code}")
            raise
