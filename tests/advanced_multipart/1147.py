...
def test_1147(s3_client, config):
    """Multipart copy 1147"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1147')
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1147 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1147: {error_code}")
            raise
