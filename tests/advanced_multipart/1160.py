...
def test_1160(s3_client, config):
    """Multipart copy 1160"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1160')
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1160 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1160: {error_code}")
            raise
