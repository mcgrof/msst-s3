...
def test_1151(s3_client, config):
    """Multipart copy 1151"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1151')
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1151 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1151: {error_code}")
            raise
