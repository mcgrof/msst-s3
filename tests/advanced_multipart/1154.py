...
def test_1154(s3_client, config):
    """Multipart copy 1154"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1154')
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1154 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1154: {error_code}")
            raise
