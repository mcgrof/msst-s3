...
def test_1153(s3_client, config):
    """Multipart copy 1153"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1153')
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1153 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1153: {error_code}")
            raise
