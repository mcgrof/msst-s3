...
def test_1144(s3_client, config):
    """Multipart copy 1144"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1144')
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1144 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1144: {error_code}")
            raise
