...
def test_1119(s3_client, config):
    """Variable parts 1119"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1119')
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1119 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1119: {error_code}")
            raise
