...
def test_1115(s3_client, config):
    """Variable parts 1115"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1115')
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1115 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1115: {error_code}")
            raise
