...
def test_1156(s3_client, config):
    """Multipart copy 1156"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1156')
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1156 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1156: {error_code}")
            raise
