...
def test_1149(s3_client, config):
    """Multipart copy 1149"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1149')
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1149 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1149: {error_code}")
            raise
