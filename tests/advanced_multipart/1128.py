#!/usr/bin/env python3
"""
Test 1128: Concurrent uploads 5

Tests 5 concurrent multipart uploads
"""
...
def test_1128(s3_client, config):
    """Concurrent uploads 5"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1128')
        s3_client.create_bucket(bucket_name)

        # Test 5 concurrent multipart uploads
        import threading
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1128 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1128: {error_code}")
            raise
