...
def test_1148(s3_client, config):
    """Multipart copy 1148"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1148')
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1148 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1148: {error_code}")
            raise
