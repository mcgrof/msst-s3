...
def test_1142(s3_client, config):
    """Multipart copy 1142"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1142')
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1142 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1142: {error_code}")
            raise
