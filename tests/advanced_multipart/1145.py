...
def test_1145(s3_client, config):
    """Multipart copy 1145"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1145')
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1145 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1145: {error_code}")
            raise
