...
def test_1157(s3_client, config):
    """Multipart copy 1157"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1157')
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1157 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1157: {error_code}")
            raise
