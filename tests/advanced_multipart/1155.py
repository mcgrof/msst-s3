...
def test_1155(s3_client, config):
    """Multipart copy 1155"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1155')
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1155 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1155: {error_code}")
            raise
