...
def test_1105(s3_client, config):
    """Variable parts 1105"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1105')
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1105 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1105: {error_code}")
            raise
