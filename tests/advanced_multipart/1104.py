...
def test_1104(s3_client, config):
    """Variable parts 1104"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1104')
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1104 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1104: {error_code}")
            raise
