#!/usr/bin/env python3
"""
Test 1121: Concurrent uploads 3

Tests 3 concurrent multipart uploads
"""
...
def test_1121(s3_client, config):
    """Concurrent uploads 3"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1121')
        s3_client.create_bucket(bucket_name)

        # Test 3 concurrent multipart uploads
        import threading
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1121 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1121: {error_code}")
            raise
