...
def test_1120(s3_client, config):
    """Variable parts 1120"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1120')
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1120 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1120: {error_code}")
            raise
