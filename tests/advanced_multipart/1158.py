...
def test_1158(s3_client, config):
    """Multipart copy 1158"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1158')
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1158 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1158: {error_code}")
            raise
