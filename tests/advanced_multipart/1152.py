...
def test_1152(s3_client, config):
    """Multipart copy 1152"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1152')
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1152 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1152: {error_code}")
            raise
