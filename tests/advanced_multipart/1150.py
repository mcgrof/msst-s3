...
def test_1150(s3_client, config):
    """Multipart copy 1150"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1150')
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1150 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1150: {error_code}")
            raise
