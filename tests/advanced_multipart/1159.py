...
def test_1159(s3_client, config):
    """Multipart copy 1159"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1159')
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1159 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1159: {error_code}")
            raise
