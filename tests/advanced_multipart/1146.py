...
def test_1146(s3_client, config):
    """Multipart copy 1146"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1146')
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1146 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1146: {error_code}")
            raise
