...
def test_1143(s3_client, config):
    """Multipart copy 1143"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1143')
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1143 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1143: {error_code}")
            raise
