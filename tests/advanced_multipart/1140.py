#!/usr/bin/env python3
"""
Test 1140: Concurrent uploads 2

Tests 2 concurrent multipart uploads
"""
...
def test_1140(s3_client, config):
    """Concurrent uploads 2"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-1140')
        s3_client.create_bucket(bucket_name)

        # Test 2 concurrent multipart uploads
        import threading
        ...
        if error_code in ['NotImplemented', 'InvalidRequest']:
            print(f"Test 1140 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 1140: {error_code}")
            raise
