...
def test_2654(s3_client, config):
    """Lambda@Edge 2654"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2654')
        s3_client.create_bucket(bucket_name)

        # Test Lambda@Edge patterns
        key = f'lambda-edge/responses/modified-2654.html'
        ...
        lambda_edge = {
            ...
            'modifications': {
                'headers_added': {
                    'X-Custom-Header': f'value-2654',
                    'X-Request-Id': f'REQ-00002654',
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2654 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2654: {error_code}")
            raise
