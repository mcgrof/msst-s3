#!/usr/bin/env python3
"""
Test 2665: Global Accelerator 2665
...
def test_2665(s3_client, config):
    """Global Accelerator 2665"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2665')
        s3_client.create_bucket(bucket_name)

        # Test Global Accelerator patterns
        key = f'global-accelerator/endpoint-2665.json'

        # Global Accelerator configuration
        accelerator_config = {
            'accelerator_id': f'ACC-002665',
            'listeners': [
            ...
            'anycast_ips': [f'192.0.2.105', f'198.51.100.105']
        }
        ...
        print(f"Global Accelerator test 2665: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2665 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2665: {error_code}")
            raise
