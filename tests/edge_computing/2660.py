...
def test_2660(s3_client, config):
    """Lambda@Edge 2660"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2660')
        s3_client.create_bucket(bucket_name)

        # Test Lambda@Edge patterns
        key = f'lambda-edge/responses/modified-2660.html'
        ...
        lambda_edge = {
            ...
            'modifications': {
                'headers_added': {
                    'X-Custom-Header': f'value-2660',
                    'X-Request-Id': f'REQ-00002660',
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2660 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2660: {error_code}")
            raise
