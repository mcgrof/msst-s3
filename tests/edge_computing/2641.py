...
def test_2641(s3_client, config):
    """Lambda@Edge 2641"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2641')
        s3_client.create_bucket(bucket_name)

        # Test Lambda@Edge patterns
        key = f'lambda-edge/responses/modified-2641.html'
        ...
        lambda_edge = {
            ...
            'modifications': {
                'headers_added': {
                    'X-Custom-Header': f'value-2641',
                    'X-Request-Id': f'REQ-00002641',
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2641 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2641: {error_code}")
            raise
