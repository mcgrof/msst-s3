...
def test_2646(s3_client, config):
    """Lambda@Edge 2646"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2646')
        s3_client.create_bucket(bucket_name)

        # Test Lambda@Edge patterns
        key = f'lambda-edge/responses/modified-2646.html'
        ...
        lambda_edge = {
            ...
            'modifications': {
                'headers_added': {
                    'X-Custom-Header': f'value-2646',
                    'X-Request-Id': f'REQ-00002646',
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2646 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2646: {error_code}")
            raise
