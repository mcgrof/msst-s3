...
def test_2644(s3_client, config):
    """Lambda@Edge 2644"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2644')
        s3_client.create_bucket(bucket_name)

        # Test Lambda@Edge patterns
        key = f'lambda-edge/responses/modified-2644.html'
        ...
        lambda_edge = {
            ...
            'modifications': {
                'headers_added': {
                    'X-Custom-Header': f'value-2644',
                    'X-Request-Id': f'REQ-00002644',
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2644 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2644: {error_code}")
            raise
