...
def test_2265(s3_client, config):
    """Delta Lake 2265"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2265')
        s3_client.create_bucket(bucket_name)

        # Test Delta Lake format
        delta_path = f'data-lake/delta/table-2265/'
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2265 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2265: {error_code}")
            raise
