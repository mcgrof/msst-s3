...
def test_2287(s3_client, config):
    """Data catalog 2287"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2287')
        s3_client.create_bucket(bucket_name)

        # Test data catalog integration
        key = f'data-lake/catalog/database-7/table-2287/data.orc'
        ...
        catalog_entry = {
            ...
            'location': f's3://{bucket_name}/data-lake/catalog/database-7/table-2287/',
        ...
        print(f"Data catalog test 2287: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2287 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2287: {error_code}")
            raise
