...
def test_2248(s3_client, config):
    """Iceberg format 2248"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2248')
        s3_client.create_bucket(bucket_name)

        # Test Apache Iceberg table format
        key = f'data-lake/iceberg/table-2248/metadata/v{i}.metadata.json'

        # Iceberg table metadata
        iceberg_metadata = {
            'format-version': 2,
            'table-uuid': f'table-2248-' + 'x' * 32,
            'location': f's3://{bucket_name}/data-lake/iceberg/table-2248',
            'schema': {
                'type': 'struct',
                'fields': [
        ...
        s3_client.client.put_object(
            ...
            Metadata={
                ...
                'format-version': '2',
                'table-name': f'table_2248'
            }
        )

        print(f"Iceberg table test 2248: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2248 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2248: {error_code}")
            raise
