...
def test_2293(s3_client, config):
    """Data catalog 2293"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2293')
        s3_client.create_bucket(bucket_name)

        # Test data catalog integration
        key = f'data-lake/catalog/database-3/table-2293/data.orc'
        ...
        catalog_entry = {
            ...
            'location': f's3://{bucket_name}/data-lake/catalog/database-3/table-2293/',
        ...
        print(f"Data catalog test 2293: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2293 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2293: {error_code}")
            raise
