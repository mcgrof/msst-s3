...
def test_2295(s3_client, config):
    """Data catalog 2295"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2295')
        s3_client.create_bucket(bucket_name)

        # Test data catalog integration
        key = f'data-lake/catalog/database-5/table-2295/data.orc'

        # Catalog metadata
        catalog_entry = {
            'database': f'database_5',
            'table': f'table_2295',
            'columns': [
            ...
            'location': f's3://{bucket_name}/data-lake/catalog/database-5/table-2295/',
        ...
        print(f"Data catalog test 2295: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2295 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2295: {error_code}")
            raise
