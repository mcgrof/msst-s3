...
def test_2244(s3_client, config):
    """Iceberg format 2244"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2244')
        s3_client.create_bucket(bucket_name)

        # Test Apache Iceberg table format
        key = f'data-lake/iceberg/table-2244/metadata/v{i}.metadata.json'

        # Iceberg table metadata
        iceberg_metadata = {
            'format-version': 2,
            'table-uuid': f'table-2244-' + 'x' * 32,
            'location': f's3://{bucket_name}/data-lake/iceberg/table-2244',
            'schema': {
                'type': 'struct',
                'fields': [
        ...
        s3_client.client.put_object(
            ...
            Metadata={
                ...
                'format-version': '2',
                'table-name': f'table_2244'
            }
        )

        print(f"Iceberg table test 2244: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2244 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2244: {error_code}")
            raise
