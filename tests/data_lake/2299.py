...
def test_2299(s3_client, config):
    """Data catalog 2299"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2299')
        s3_client.create_bucket(bucket_name)

        # Test data catalog integration
        key = f'data-lake/catalog/database-9/table-2299/data.orc'

        # Catalog metadata
        catalog_entry = {
            'database': f'database_9',
            'table': f'table_2299',
            'columns': [
            ...
            'location': f's3://{bucket_name}/data-lake/catalog/database-9/table-2299/',
        ...
        print(f"Data catalog test 2299: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2299 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2299: {error_code}")
            raise
