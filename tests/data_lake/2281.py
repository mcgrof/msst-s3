...
def test_2281(s3_client, config):
    """Data catalog 2281"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2281')
        s3_client.create_bucket(bucket_name)

        # Test data catalog integration
        key = f'data-lake/catalog/database-1/table-2281/data.orc'

        # Catalog metadata
        catalog_entry = {
            'database': f'database_1',
            'table': f'table_2281',
            'columns': [
            ...
            'location': f's3://{bucket_name}/data-lake/catalog/database-1/table-2281/',
        ...
        print(f"Data catalog test 2281: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2281 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2281: {error_code}")
            raise
