...
def test_2294(s3_client, config):
    """Data catalog 2294"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2294')
        s3_client.create_bucket(bucket_name)

        # Test data catalog integration
        key = f'data-lake/catalog/database-4/table-2294/data.orc'
        ...
        catalog_entry = {
            ...
            'location': f's3://{bucket_name}/data-lake/catalog/database-4/table-2294/',
        ...
        print(f"Data catalog test 2294: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2294 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2294: {error_code}")
            raise
