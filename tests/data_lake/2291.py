...
def test_2291(s3_client, config):
    """Data catalog 2291"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2291')
        s3_client.create_bucket(bucket_name)

        # Test data catalog integration
        key = f'data-lake/catalog/database-1/table-2291/data.orc'
        ...
        catalog_entry = {
            ...
            'location': f's3://{bucket_name}/data-lake/catalog/database-1/table-2291/',
        ...
        print(f"Data catalog test 2291: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2291 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2291: {error_code}")
            raise
