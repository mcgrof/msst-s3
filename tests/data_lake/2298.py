...
def test_2298(s3_client, config):
    """Data catalog 2298"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2298')
        s3_client.create_bucket(bucket_name)

        # Test data catalog integration
        key = f'data-lake/catalog/database-8/table-2298/data.orc'

        # Catalog metadata
        catalog_entry = {
            'database': f'database_8',
            'table': f'table_2298',
            'columns': [
            ...
            'location': f's3://{bucket_name}/data-lake/catalog/database-8/table-2298/',
        ...
        print(f"Data catalog test 2298: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2298 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2298: {error_code}")
            raise
