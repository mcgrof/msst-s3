...
def test_2962(s3_client, config):
    """Direct Connect 2962"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2962')
        s3_client.create_bucket(bucket_name)

        # Test Direct Connect configuration
        key = f'direct-connect/vif-2962/config.json'

        # Virtual Interface configuration
        vif_config = {
            'vif_id': f'dxvif-00000b92',
            'connection_id': f'dxcon-00000002',
            'vif_type': ['private', 'public', 'transit'][i % 3],
            'vlan': 100 + i,
            'customer_address': f'192.168.146.1/30',
            'amazon_address': f'192.168.146.2/30',
            'bgp_config': {
                'asn': 65000 + i,
                'auth_key': hashlib.md5(f'auth-2962'.encode()).hexdigest()[:16],
                'prefixes_advertised': [
                    f'10.146.0.0/16',
                    f'172.18.0.0/16'
                ]
        ...
        print(f"Direct Connect test 2962: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2962 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2962: {error_code}")
            raise
