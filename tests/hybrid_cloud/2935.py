...
def test_2935(s3_client, config):
    """On-premises 2935"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2935')
        s3_client.create_bucket(bucket_name)

        # Test on-premises integration
        key = f'hybrid/on-prem/gateway-2935/config.json'

        # Storage Gateway configuration
        gateway_config = {
            'gateway_type': ['file', 'volume', 'tape'][i % 3],
            'gateway_id': f'SGW-00002935',
            'on_premises': {
                'location': f'datacenter-0',
                'ip_address': f'10.119.119.119',
        ...
        print(f"On-premises integration test 2935: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2935 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2935: {error_code}")
            raise
