...
def test_2932(s3_client, config):
    """On-premises 2932"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2932')
        s3_client.create_bucket(bucket_name)

        # Test on-premises integration
        key = f'hybrid/on-prem/gateway-2932/config.json'

        # Storage Gateway configuration
        gateway_config = {
            'gateway_type': ['file', 'volume', 'tape'][i % 3],
            'gateway_id': f'SGW-00002932',
            'on_premises': {
                'location': f'datacenter-2',
                'ip_address': f'10.116.116.116',
        ...
        print(f"On-premises integration test 2932: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2932 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2932: {error_code}")
            raise
