...
def test_2949(s3_client, config):
    """DataSync 2949"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2949')
        s3_client.create_bucket(bucket_name)

        # Test AWS DataSync operations
        key = f'datasync/tasks/task-2949/status.json'

        # DataSync task configuration
        datasync_task = {
            'task_arn': f'arn:aws:datasync:us-east-1:123456789012:task/task-00002949',
            'source_location': {
                'type': ['nfs', 'smb', 's3', 'efs'][i % 4],
                'uri': f'{["nfs", "smb", "s3", "efs"][i % 4]}://source-2949.example.com/share'
            },
            'destination_location': {
                'type': 's3',
                'bucket': bucket_name,
                'prefix': f'datasync-2949/'
            },
            'options': {
        ...
        print(f"DataSync operation test 2949: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2949 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2949: {error_code}")
            raise
