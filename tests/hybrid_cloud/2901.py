#!/usr/bin/env python3
"""
Test 2901: Multi-cloud sync 2901
...
def test_2901(s3_client, config):
    """Multi-cloud sync 2901"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2901')
        s3_client.create_bucket(bucket_name)

        # Test multi-cloud storage sync
        key = f'multi-cloud/sync/{source_cloud}-to-{target_cloud}/data-2901.dat'
        ...
        sync_config = {
            'source': {
                'provider': 'azure',
                'region': 'us-east-1',
                'bucket': f'source-bucket-2901',
                'endpoint': f'https://s3.{source_cloud}.com'
            },
            'target': {
                'provider': 'gcp',
                'region': 'europe-west1',
                'bucket': f'target-bucket-2901',
        ...
        print(f"Multi-cloud sync (azure → gcp) test 2901: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2901 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2901: {error_code}")
            raise
