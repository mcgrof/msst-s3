...
def test_2982(s3_client, config):
    """Outposts 2982"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2982')
        s3_client.create_bucket(bucket_name)

        # Test AWS Outposts configuration
        key = f'outposts/outpost-2982/config.json'

        # Outposts configuration
        outpost_config = {
            'outpost_id': f'op-000000000ba6',
            ...
            's3_on_outposts': {
                'buckets': [
                    {
                        'name': f'outpost-bucket-2982-{j}',
                        'capacity_gb': 1000,
                        'storage_class': 'OUTPOSTS'
                    } for j in range(3)
                ],
                'access_points': [f'ap-outpost-2982-{j}' for j in range(2)],
                'endpoints': [f'https://op-000000000ba6.s3-outposts.amazonaws.com']
            },
            'connectivity': {
        ...
        print(f"AWS Outposts test 2982: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2982 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2982: {error_code}")
            raise
