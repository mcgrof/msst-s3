...
def test_2997(s3_client, config):
    """Outposts 2997"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2997')
        s3_client.create_bucket(bucket_name)

        # Test AWS Outposts configuration
        key = f'outposts/outpost-2997/config.json'

        # Outposts configuration
        outpost_config = {
            'outpost_id': f'op-000000000bb5',
            ...
            's3_on_outposts': {
                'buckets': [
                    {
                        'name': f'outpost-bucket-2997-{j}',
                        'capacity_gb': 1000,
                        'storage_class': 'OUTPOSTS'
                    } for j in range(3)
                ],
                'access_points': [f'ap-outpost-2997-{j}' for j in range(2)],
                'endpoints': [f'https://op-000000000bb5.s3-outposts.amazonaws.com']
            },
            'connectivity': {
        ...
        print(f"AWS Outposts test 2997: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2997 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2997: {error_code}")
            raise
