...
def test_2988(s3_client, config):
    """Outposts 2988"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2988')
        s3_client.create_bucket(bucket_name)

        # Test AWS Outposts configuration
        key = f'outposts/outpost-2988/config.json'

        # Outposts configuration
        outpost_config = {
            'outpost_id': f'op-000000000bac',
            'site_id': f'site-00000008',
            'availability_zone': f'us-east-1-op-1a',
            'capacity': {
                'compute': {
                    'ec2_instances': {
                        'm5.large': 10 * (i % 5 + 1),
            ...
            's3_on_outposts': {
                'buckets': [
                    {
                        'name': f'outpost-bucket-2988-{j}',
                        'capacity_gb': 1000,
                        'storage_class': 'OUTPOSTS'
                    } for j in range(3)
                ],
                'access_points': [f'ap-outpost-2988-{j}' for j in range(2)],
                'endpoints': [f'https://op-000000000bac.s3-outposts.amazonaws.com']
            },
            'connectivity': {
        ...
        print(f"AWS Outposts test 2988: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2988 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2988: {error_code}")
            raise
