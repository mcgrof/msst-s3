...
def test_2978(s3_client, config):
    """Direct Connect 2978"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2978')
        s3_client.create_bucket(bucket_name)

        # Test Direct Connect configuration
        key = f'direct-connect/vif-2978/config.json'

        # Virtual Interface configuration
        vif_config = {
            'vif_id': f'dxvif-00000ba2',
            'connection_id': f'dxcon-00000008',
            'vif_type': ['private', 'public', 'transit'][i % 3],
            'vlan': 100 + i,
            'customer_address': f'192.168.162.1/30',
            'amazon_address': f'192.168.162.2/30',
            'bgp_config': {
                'asn': 65000 + i,
                'auth_key': hashlib.md5(f'auth-2978'.encode()).hexdigest()[:16],
                'prefixes_advertised': [
                    f'10.162.0.0/16',
                    f'172.18.0.0/16'
                ]
        ...
        print(f"Direct Connect test 2978: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2978 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2978: {error_code}")
            raise
