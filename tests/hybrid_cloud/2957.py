...
def test_2957(s3_client, config):
    """DataSync 2957"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2957')
        s3_client.create_bucket(bucket_name)

        # Test AWS DataSync operations
        key = f'datasync/tasks/task-2957/status.json'

        # DataSync task configuration
        datasync_task = {
            'task_arn': f'arn:aws:datasync:us-east-1:123456789012:task/task-00002957',
            'source_location': {
                'type': ['nfs', 'smb', 's3', 'efs'][i % 4],
                'uri': f'{["nfs", "smb", "s3", "efs"][i % 4]}://source-2957.example.com/share'
            },
            'destination_location': {
                'type': 's3',
                'bucket': bucket_name,
                'prefix': f'datasync-2957/'
            },
            'options': {
        ...
        print(f"DataSync operation test 2957: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2957 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2957: {error_code}")
            raise
