...
def test_2954(s3_client, config):
    """DataSync 2954"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2954')
        s3_client.create_bucket(bucket_name)

        # Test AWS DataSync operations
        key = f'datasync/tasks/task-2954/status.json'

        # DataSync task configuration
        datasync_task = {
            'task_arn': f'arn:aws:datasync:us-east-1:123456789012:task/task-00002954',
            'source_location': {
                'type': ['nfs', 'smb', 's3', 'efs'][i % 4],
                'uri': f'{["nfs", "smb", "s3", "efs"][i % 4]}://source-2954.example.com/share'
            },
            'destination_location': {
                'type': 's3',
                'bucket': bucket_name,
                'prefix': f'datasync-2954/'
            },
            'options': {
        ...
        print(f"DataSync operation test 2954: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2954 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2954: {error_code}")
            raise
