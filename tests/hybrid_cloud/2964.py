...
def test_2964(s3_client, config):
    """Direct Connect 2964"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2964')
        s3_client.create_bucket(bucket_name)

        # Test Direct Connect configuration
        key = f'direct-connect/vif-2964/config.json'

        # Virtual Interface configuration
        vif_config = {
            'vif_id': f'dxvif-00000b94',
            'connection_id': f'dxcon-00000004',
            'vif_type': ['private', 'public', 'transit'][i % 3],
            'vlan': 100 + i,
            'customer_address': f'192.168.148.1/30',
            'amazon_address': f'192.168.148.2/30',
            'bgp_config': {
                'asn': 65000 + i,
                'auth_key': hashlib.md5(f'auth-2964'.encode()).hexdigest()[:16],
                'prefixes_advertised': [
                    f'10.148.0.0/16',
                    f'172.20.0.0/16'
                ]
        ...
        print(f"Direct Connect test 2964: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2964 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2964: {error_code}")
            raise
