...
def test_2977(s3_client, config):
    """Direct Connect 2977"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2977')
        s3_client.create_bucket(bucket_name)

        # Test Direct Connect configuration
        key = f'direct-connect/vif-2977/config.json'

        # Virtual Interface configuration
        vif_config = {
            'vif_id': f'dxvif-00000ba1',
            'connection_id': f'dxcon-00000007',
            'vif_type': ['private', 'public', 'transit'][i % 3],
            'vlan': 100 + i,
            'customer_address': f'192.168.161.1/30',
            'amazon_address': f'192.168.161.2/30',
            'bgp_config': {
                'asn': 65000 + i,
                'auth_key': hashlib.md5(f'auth-2977'.encode()).hexdigest()[:16],
                'prefixes_advertised': [
                    f'10.161.0.0/16',
                    f'172.17.0.0/16'
                ]
        ...
        print(f"Direct Connect test 2977: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2977 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2977: {error_code}")
            raise
