...
def test_2453(s3_client, config):
    """RT analytics 2453"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2453')
        ...
        print(f"Real-time analytics test 2453: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2453 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2453: {error_code}")
            raise
