...
def test_2449(s3_client, config):
    """RT analytics 2449"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2449')
        ...
        print(f"Real-time analytics test 2449: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2449 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2449: {error_code}")
            raise
