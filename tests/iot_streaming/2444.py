...
def test_2444(s3_client, config):
    """RT analytics 2444"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2444')
        ...
        print(f"Real-time analytics test 2444: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2444 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2444: {error_code}")
            raise
