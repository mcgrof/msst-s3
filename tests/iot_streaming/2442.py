...
def test_2442(s3_client, config):
    """RT analytics 2442"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2442')
        ...
        print(f"Real-time analytics test 2442: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2442 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2442: {error_code}")
            raise
