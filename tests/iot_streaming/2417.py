...
def test_2417(s3_client, config):
    """IoT ingestion 2417"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2417')
        s3_client.create_bucket(bucket_name)

        # Test IoT device data ingestion
        device_id = f'IOT-DEVICE-00002417'
        ...
        print(f"IoT ingestion test 2417: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2417 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2417: {error_code}")
            raise
