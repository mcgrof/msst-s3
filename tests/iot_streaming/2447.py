...
def test_2447(s3_client, config):
    """RT analytics 2447"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2447')
        ...
        print(f"Real-time analytics test 2447: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2447 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2447: {error_code}")
            raise
