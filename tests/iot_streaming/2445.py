...
def test_2445(s3_client, config):
    """RT analytics 2445"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2445')
        ...
        print(f"Real-time analytics test 2445: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2445 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2445: {error_code}")
            raise
