...
def test_2446(s3_client, config):
    """RT analytics 2446"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2446')
        ...
        print(f"Real-time analytics test 2446: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2446 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2446: {error_code}")
            raise
