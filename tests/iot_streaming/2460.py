...
def test_2460(s3_client, config):
    """RT analytics 2460"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2460')
        ...
        print(f"Real-time analytics test 2460: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2460 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2460: {error_code}")
            raise
