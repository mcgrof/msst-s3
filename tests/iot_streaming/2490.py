...
def test_2490(s3_client, config):
    """CDC pattern 2490"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2490')
        ...
        cdc_event = {
            ...
            'timestamp': time.time(),
            'primary_key': {'id': f'ID-00002490'},
            'before': {'id': f'ID-00002490', 'value': 'old_value'} if i % 3 != 0 else None,
            'after': {'id': f'ID-00002490', 'value': 'new_value'} if i % 3 != 2 else None,
            'source': {
                'version': '1.9.0',
                'connector': 'mysql',
                'server_id': 1,
        ...
        # Store CDC event
        key = f'cdc/{table_name}/{int(time.time() * 1000)}-2490.json'
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2490 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2490: {error_code}")
            raise
