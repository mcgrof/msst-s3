...
def test_2474(s3_client, config):
    """Event sourcing 2474"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2474')
        s3_client.create_bucket(bucket_name)

        # Test event sourcing pattern
        aggregate_id = f'AGG-00002474'
        ...
        print(f"Event sourcing test 2474: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2474 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2474: {error_code}")
            raise
