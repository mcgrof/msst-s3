...
def test_2402(s3_client, config):
    """IoT ingestion 2402"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2402')
        s3_client.create_bucket(bucket_name)

        # Test IoT device data ingestion
        device_id = f'IOT-DEVICE-00002402'
        ...
        print(f"IoT ingestion test 2402: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2402 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2402: {error_code}")
            raise
