...
def test_2401(s3_client, config):
    """IoT ingestion 2401"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2401')
        s3_client.create_bucket(bucket_name)

        # Test IoT device data ingestion
        device_id = f'IOT-DEVICE-00002401'
        ...
        print(f"IoT ingestion test 2401: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2401 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2401: {error_code}")
            raise
