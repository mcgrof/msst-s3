...
def test_2477(s3_client, config):
    """Event sourcing 2477"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2477')
        s3_client.create_bucket(bucket_name)

        # Test event sourcing pattern
        aggregate_id = f'AGG-00002477'
        ...
        print(f"Event sourcing test 2477: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2477 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2477: {error_code}")
            raise
