...
def test_2461(s3_client, config):
    """Event sourcing 2461"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2461')
        s3_client.create_bucket(bucket_name)

        # Test event sourcing pattern
        aggregate_id = f'AGG-00002461'
        ...
        print(f"Event sourcing test 2461: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2461 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2461: {error_code}")
            raise
