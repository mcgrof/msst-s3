...
def test_2441(s3_client, config):
    """RT analytics 2441"""
    fixture = TestFixture(s3_client, config)
    bucket_name = None

    try:
        bucket_name = fixture.generate_bucket_name('test-2441')
        ...
        print(f"Real-time analytics test 2441: ✓")
        ...
        if error_code in ['NotImplemented', 'InvalidRequest', 'UnsupportedOperation']:
            print(f"Test 2441 - Feature not supported: {error_code}")
        else:
            print(f"Error in test 2441: {error_code}")
            raise
